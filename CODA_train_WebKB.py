...
parser.add_argument('--dataset', type=str, default='texas', help='cora citeseer pubmed wisconsin texas cornell')
parser.add_argument('--model', type=str, default='GIN', help='GAT, GCN, GIN, APPNP, SAGE')
parser.add_argument('--epochs', type=int, default=200, help='Number of epochs to train.')  # 200
parser.add_argument('--patience', type=int, default=20, help='Patience')  # 20
parser.add_argument('--lr', type=float, default=0.005, help='Initial learning rate.')
parser.add_argument('--weight_decay', type=float, default=5e-4, help='Weight decay (L2 loss on parameters).')
parser.add_argument('--hidden', type=int, default=8, help='Dropout rate (1 - keep probability).')

parser.add_argument('--s', type=int, default=5, help='')
parser.add_argument('--T0', type=float, default=0, help='T_O: Temperature coefficient')
parser.add_argument('--T1', type=float, default=1.6, help='T_C: Temperature coefficient')
parser.add_argument('--dropout0', type=float, default=1., help='d_0: Dropout rate (1 - keep probability).')
parser.add_argument('--dropout1', type=float, default=1., help='d_1: Dropout rate (1 - keep probability).')
parser.add_argument('--self_dropout0', type=float, default=1, help='d_2: Dropout rate (1 - keep probability).')
parser.add_argument('--self_dropout1', type=float, default=0.3, help='d_3: Dropout rate (1 - keep probability).')
parser.add_argument('--prior_model', type=str, default='gemo', help='pre-GNNs: grand, gemo')
...
all_test_acc = []
for i in range(10):
    g, adj, features, labels, train_mask, val_mask, test_mask = load_data_webkb(args.dataset, f'./data/new_data/'
                                                                f'{args.dataset}/{args.dataset}_split_0.6_0.2_{i}.npz',
                                                                                   0.6, 0.2)
    ...
    if args.prior_model == 'grand' or args.prior_model == 'gemo':
    ...
    # the auxiliary graph
    new_g = graph_augmentation(cat_dis, args.s * (len(g.edges()[0] - len(g.nodes())))).to(device)
